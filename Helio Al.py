...
def load_angular_file(path):
    """
    Formatos admitidos (texto; líneas que empiezan por '#' se ignoran):
...
def sigma_I_total(theta, I, dtheta, err_percent=None, usar_discretizacion=True):
    """
    sigma_tot^2 = (I * err_percent/100)^2  +  (|dI/dθ| * dθ / sqrt(12))^2
    Si no hay err_percent, devuelve solo la de discretización.
    """
...
def cruce_dos_puntos(x1, x2, y1, y2, s1, s2, yf, syf):
    """
    Interpolación lineal entre (x1,y1) y (x2,y2) para hallar xf con y(xf)=yf.
    Propaga incertidumbre desde y1, y2 y yf (independientes).
    Devuelve: xf, s_xf, d(xf)/d(yf).
    """
    dx = x2 - x1
    Q  = y2 - y1
    if Q == 0:
...
def angular_levels(path, niveles=(np.e**-1, 0.5, 0.1, 0.01),
    ...
    """
    Calcula los ángulos θ_f donde I(θ)=f·I_pico para f en 'niveles',
    usando SIEMPRE interpolación lineal a dos puntos y propagación.
    I_pico y σ(I_pico) se toman del bin de máximo (si hay err%).
    """
...
if __name__ == "__main__":
    path = Path(r"C:\Users\alber\OneDrive\Escritorio\Física Nuclear\TFM\Bibliografía\Láminas delgadas\Estudio dispersión angular\Validaciones Helio\Distribución Ang_Helio_Al_MeV_21_tab.lis")
